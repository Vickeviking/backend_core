...
fn basic_authentication(headers: &HeaderMap) -> Result<Credentials, anyhow::Error> {
    // The header value, if present, must be valid UTF8 string
...
async fn validate_credentials(
    ...
    // Even if password match user_id would be None,
    // We verify password hash in both cases to prevent timing attack so each path takes equal
    // time.
...
async fn get_stored_credentials(
    ...
    // fetch credentials via username
    let row = sqlx::query!(
        r#"
